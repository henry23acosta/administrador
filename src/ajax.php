...
if (isset($_GET['q'])) {
    $datos = array();
    $nombre = $_GET['q'];
    echo "<script>console.log('Console: " + $nombre + "' );</script>";
...
if (isset($_POST['cambio'])) {
    if (empty($_POST['actual']) || empty($_POST['nueva'])) {
        ...
        $consulta = mysqli_query($conexion, "SELECT * FROM usuario WHERE password = '$actual' AND idusuario = $id");
        $result = mysqli_num_rows($consulta);
        if ($result == 1) {
            $query = mysqli_query($conexion, "UPDATE usuario SET password = '$nueva' WHERE idusuario = $id");
            if ($query) {
                $msg = 'ok';
            }else{
                $msg = 'error';
